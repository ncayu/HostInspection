...
- 仅支持单机本地巡检，不包含远程批量执行能力。
- 所有逻辑集中在一个 Bash 文件中，后续扩展时维护成本较高。
- 健康分基于 HTML 状态标签数量计算，属于启发式评分。
- 服务检查采用固定服务名列表，可能无法覆盖自定义服务实例。
- SSH 配置读取的是 `/etc/ssh/sshd_config` 中显式配置项，未完整解析 `Include`、条件块及最终生效配置。
- 页面使用 CDN 图标资源，默认并非完全离线。
- 导出的 `.doc` 本质上是 Word 可识别的 HTML 文档，而不是原生 OOXML `.docx` 文件。

## 后续改进建议

- 将巡检项拆分为独立模块，便于维护和按需启用。
- 增加命令行参数，例如输出目录、主机标签和巡检模块选择。
- 增加 JSON 或 CSV 等结构化输出格式。
- 支持配置化阈值和自定义服务列表。
- 使用 `sshd -T` 获取 SSH 最终生效配置。
- 将前端静态资源内嵌或本地化，支持完全离线报告。
- 增加多主机报告汇总及历史趋势对比。
- 增加 ShellCheck 和自动化测试流程。
