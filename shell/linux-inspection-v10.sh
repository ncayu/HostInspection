...
if command -v dmidecode &>/dev/null && [ "$(id -u)" -eq 0 ]; then
    MEM_MODULES=$(dmidecode -t memory 2>/dev/null | grep -E "Size:|Type:|Speed:|Manufacturer:|Locator:" | grep -v "No Module\|Unknown\|Empty" | html_escape)
fi
...
df -h 2>/dev/null | awk 'NR>1 && !/tmpfs|devtmpfs|overlay/ {print $1, $6, $2, $3, $4, $5}' | while read -r fs mount size used avail use_pct; do
    ...
    echo "<tr><td>${fs}</td><td>${mount}</td><td>${size}</td><td>${used}</td><td>${avail}</td><td><div class=\"seg-bar\">${seg_html}<span class=\"seg-pct\">${use_pct}%</span></div></td><td>${status}</td></tr>" >> "$HTML_REPORT"
done
...
df -i 2>/dev/null | awk 'NR>1 && !/tmpfs|devtmpfs|overlay/ {print $1, $6, $2, $3, $4, $5}' | while read -r fs mount itotal iused iavail iuse; do
    [ -z "$fs" ] && continue
    echo "<tr><td>${fs}</td><td>${mount}</td><td>${itotal}</td><td>${iused}</td><td>${iavail}</td><td>${iuse}</td></tr>" >> "$HTML_REPORT"
done
echo "</table>" >> "$HTML_REPORT"
...
# 完成
# ------------------------------------------------------------
echo -e "${GREEN}[4/4] Linux 系统巡检完成 (v10)${NC}"
